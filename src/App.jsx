...
export default function App() {
  ...
  const runAIAnalysis = async () => {
    if (hillaryProducts.length === 0) return;
    setLoading(true);
    setStep('analyzing');
    setError(null);
    await saveProfile();
    ...
    const prompt = `Ти - професійний ШІ-косметолог HiLLARY Cosmetics. 
    ТВОЄ ЗАВДАННЯ: 
    ...
    const makeRequest = async (retries = 0) => {
      try {
        ...
        // Очищення JSON від Markdown блоків
        const cleanJson = text.replace(/```json|```/g, "").trim();
        const aiResponse = JSON.parse(cleanJson);
        
        if (!aiResponse.is_human_face) {
          setError(aiResponse.rejection_reason || "Ми не змогли розпізнати обличчя.");
          setStep('upload');
          return;
        }
        ...
        if (user) {
          await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, 'history'), {
            ...
            analysis: aiResponse,
            recommendationIds: matchedRecs.map(r => r.id),
            userPhoto: image
          });
        }
        setStep('results');
